  ...
  return (
    <div className="bg-bg-primary md:border border-border-tertiary rounded-xl p-4 md:p-6 mb-8 min-h-[500px]">
      ...
      <div className="flex overflow-x-auto gap-2 mb-6 pb-2 scrollbar-none">
        ...
        <button
          type="button"
          className="px-3 py-1.5 rounded-full text-[12px] font-medium text-text-secondary border border-border-tertiary bg-bg-primary flex items-center gap-1.5 hover:bg-bg-secondary ml-auto shrink-0 transition-all"
        >
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {templates.map((template) => (
            <div
              key={template.id}
              className="group border border-border-tertiary rounded-xl overflow-hidden hover:border-[#AFA9EC] hover:shadow-sm transition-all bg-bg-primary flex flex-col"
            >
              <div
                className={`bg-bg-secondary flex items-center justify-center relative overflow-hidden ${
                  template.aspectRatio === '9:16'
                    ? 'aspect-[9/16] max-h-60'
                    : template.aspectRatio === '1:1'
                      ? 'aspect-square max-h-60'
                      : 'aspect-video'
                }`}
              >
                <div className="absolute inset-0 bg-gradient-to-b from-black/5 to-transparent z-0" />
                {template.thumbnail.startsWith('http') ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    src={template.thumbnail}
                    alt=""
                    className="relative z-10 w-full h-full object-cover scale-100 group-hover:scale-105 transition-transform duration-300"
                  />
                ) : (
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center z-20 gap-2">
                  <button
                    type="button"
                    className="bg-brand-primary text-white px-4 py-2 rounded-lg text-[13px] font-medium hover:bg-brand-hover hover:scale-105 transition-all shadow-lg flex items-center gap-2"
                  >
              ...
              <div className="p-3.5 border-t border-border-tertiary">
                <h3 className="text-[13px] font-medium text-text-primary mb-1">{template.name}</h3>
