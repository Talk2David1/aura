...
type SubFlow = 'main' | 'otp_signup' | 'email_verify' | 'forgot';
...
export function AuthView({ onLogin }: AuthViewProps) {
  const [subFlow, setSubFlow] = useState<SubFlow>('main');
  ...
  if (subFlow === 'otp_signup') {
    return (
      <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center bg-bg-tertiary overflow-y-auto py-6 md:py-0">
        ...
        <OtpSignUpFlow onBack={resetMain} onSuccess={onLogin} />
      </div>
    );
  }

  if (subFlow === 'email_verify') {
    return (
      <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center bg-bg-tertiary overflow-y-auto py-6 md:py-0">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-brand-primary/20 rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-coral-primary/10 rounded-full blur-[100px] pointer-events-none" />
        <EmailVerificationFlow onBack={resetMain} />
      </div>
    );
  }
  ...
  if (subFlow === 'forgot') {
    return (
      <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center bg-bg-tertiary overflow-y-auto py-6 md:py-0">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-brand-primary/20 rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-coral-primary/10 rounded-full blur-[100px] pointer-events-none" />
        <ForgotPasswordFlow onBack={resetMain} onSuccess={onLogin} />
      </div>
    );
  }
      ...
      <div className="bg-bg-primary border border-border-tertiary w-full max-w-[420px] md:max-w-[390px] rounded-2xl p-6 md:p-7 shadow-xl relative z-10 mx-4 md:max-h-[90vh] md:overflow-y-auto">
        ...
        <div className="space-y-3">
          <button
            type="button"
            disabled
            title="GitHub sign-in is not enabled on this API"
            className="w-full flex items-center justify-center gap-3 px-4 py-2 bg-bg-secondary border border-border-tertiary rounded-xl text-[14px] font-medium text-text-tertiary opacity-50 cursor-not-allowed"
          >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-4 py-2 bg-bg-secondary border border-border-tertiary rounded-xl text-[14px] font-medium text-text-primary hover:bg-border-tertiary transition-colors"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-3 md:space-y-2.5">
          {error ? (
            <p className="text-[12px] text-coral-dark bg-coral-light/50 border border-coral-primary/30 rounded-lg px-3 py-2">
              {error}
            </p>
          ) : null}
          ...
          {isSignUp ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label htmlFor="auth-firstname" className="block text-[12px] text-text-secondary mb-1.5 font-medium ml-1">
                  First Name
                </label>
              ...
              <div>
                <label htmlFor="auth-lastname" className="block text-[12px] text-text-secondary mb-1.5 font-medium ml-1">
                  Last Name
                </label>
                <div className="relative">
                  <User size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-tertiary" />
          ...
          <div>
            <label htmlFor="auth-email" className="block text-[12px] text-text-secondary mb-1.5 font-medium ml-1">
              Email address
            </label>
            <div className="relative">
          ...
          {isSignUp ? (
            <label htmlFor="auth-terms" className="flex items-start gap-2.5 text-[12px] text-text-tertiary leading-relaxed cursor-pointer">
              <input
                id="auth-terms"
                name="terms"
                type="checkbox"
          ...
          {!isSignUp ? (
            <div className="flex flex-col gap-2 pt-1">
              <button
                type="button"
                onClick={() => { setSubFlow('forgot'); setError(null); }}
                className="text-[12px] text-brand-primary hover:text-brand-hover font-medium text-center"
              >
                Forgot password?
              </button>
              <button
                type="button"
                onClick={() => { setSubFlow('email_verify'); setError(null); }}
                className="text-[12px] text-text-tertiary hover:text-text-secondary text-center"
              >
          ...
          {isSignUp ? (
            <button
              type="button"
              onClick={() => { setSubFlow('otp_signup'); setError(null); }}
              className="w-full py-2 text-[13px] text-brand-primary hover:text-brand-hover font-medium border border-border-tertiary rounded-xl bg-bg-secondary"
            >
